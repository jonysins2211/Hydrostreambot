...
async def watch_handler(request: web.Request):
    try:
        ...
        logging.warning(f"Error in watch_handler: {e}")
        return web.Response(status=500, text="Internal error in watch handler.")
...
async def download_handler(request: web.Request):
    try:
        ...
        logging.warning(f"Error in download_handler: {e}")
        return web.Response(status=500, text="Internal error in download handler.")
    except Exception as e:
        logging.critical(e)
        logging.debug(traceback.format_exc())
        return web.Response(status=500, text="Internal Server Error")
...
async def stream_handler(request: web.Request):
    try:
        ...
        logging.warning(f"Error in stream_handler: {e}")
        return web.Response(status=500, text="Internal error in stream handler.")
    except Exception as e:
        logging.critical(e)
        logging.debug(traceback.format_exc())
        return web.Response(status=500, text="Internal Server Error")
...
async def media_streamer(request: web.Request, db_id: str):
    try:
        ...
        body = tg_connect.yield_file(
            file_id, client_index, offset, first_part_cut,
        ...
        logging.error(f"Unexpected error in media_streamer: {e}")
        logging.debug(traceback.format_exc())
        return web.Response(status=500, text="Internal Server Error")
